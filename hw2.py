...
def scale_vecs(vecdict):
    ...
    True
    '''
    return [ (1/x) * vecdict[x] for x in vecdict ]
...
def GF2_span(D, L):
    ...
    True
    '''
    comb =  [list(x) for x in product([0,one], repeat=len(L))]
    #res = []
    #for x in comb:
    #    res.append( vec_sum( [ x[y] * L[y] for y in range(len(L)) ] , D) )
    res = [ vec_sum( [x[y] * L[y] for y in range(len(L)) ], D) for x in comb]
    ##comb = [ vec_sum(x[y] * L[y]) for y in range(len(L)) for x in comb]
    return res
