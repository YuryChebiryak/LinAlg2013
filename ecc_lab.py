...
def find_error(e):
    ...
    return Vec(H.D[1], { x:one for x in H.D[1] if mat2coldict(H)[x] == e })
...
original = Vec({0,1,2,3}, {0:0, 1:one, 2:0, 3:one}) #[0,one,0,one] # R * code_word
...
#bits2str(mat2bits(R * (find_error_matrix(H * CTILDE) + CTILDE)))
