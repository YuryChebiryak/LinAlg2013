...
a = [ list2vec([2,4,3,5,0]), list2vec([4,-2,-5,4,0]), list2vec([-8,14,21,-2,0]), \
     list2vec([-1,-4,-4,0,0]), list2vec([-2,-18,-19,-6,0]), list2vec([5,-3,1,-5,2]) ]

#for x in basis(a): print(x)
...
def subset_basis(vlist):
    ...
    '''
    return [ vlist[x] for x in range(0, len(vlist)) if orthogonalize(vlist)[x] * orthogonalize(vlist)[x] > 1e-20 ]

#for x in subset_basis(a): print(x)
...
c1 = least_squares_Q1.transpose() * least_squares_b1
from solver import solve
x = solve(least_squares_R1, c1)
...
c2 = least_squares_Q2.transpose() * least_squares_b2
x2 = solve(least_squares_R2, c2)
#print(x2)

x_hat_2 = Vec({0, 1},{0: 2.499999999999997, 1: 2.6666666666666776})
...
def QR_solve(A, b):
    ...
    rowlist = [ mat2rowdict(R)[x] for x in sorted(R.D[0], key=repr)]
    #print(rowlist)
    bb = orthogonal_vec2rep(Q.transpose(), b)
    #print(bb)
    x = triangular_solve(rowlist, label_list, bb)
    return x
