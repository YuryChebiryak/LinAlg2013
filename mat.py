...
def add(A, B):
    ...
    for key in ( A.f.keys() | B.f.keys() ):
        setitem(res, key, getitem(A, key) + getitem(B, key))
    return res

def scalar_mul(M, alpha):
    "Returns the product of scalar alpha with M" 
    return Mat(M.D, { key: alpha * M.f[key] for key in M.f } )
...
def vector_matrix_mul(v, M):
    ...
    dict = {}
    for m in M.D[1]:
        s = sum( getitem(M, (n, m)) * v[n] for n in M.D[0] )
#        for n in M.D[0]:
#            sum = sum + getitem(M, (n, m)) * v[n]
        dict[m] = s
    return Vec(M.D[1], dict)
...
def matrix_vector_mul(M, v):
    ...
    dict ={}
    for m in M.D[0]:
        s = sum( getitem(M, (m,n)) * v[n] for n in M.D[1] )
        dict[m] = s
...
def matrix_matrix_mul(A, B):
    ...
    for n in A.D[0]:
        for k in B.D[1]:
            s = sum( getitem(A, (n, m)) * getitem(B, (m, k)) for m in A.D[1] )
            setitem(res, (n,k), s)
    return res
