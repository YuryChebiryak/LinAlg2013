...
def lin_comb_mat_vec_mult(M, v):
    ...
    res = Vec(M.D[0], {})
    for y in M.D[1]:
        res = res + v[y]* mat2coldict(M)[y]#* v[y]
    #return sum(  mat2coldict(M)[x] * v[x] for x in M.D[1] if x in v and x in mat2coldict(M))
    return res
...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    rows = mat2rowdict(M)
    res = Vec(M.D[0], { x:rows[x] * v for x in M.D[0] })
    return res
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    cols = mat2coldict(M)
    return Vec(M.D[1], { x: v*cols[x] for x in M.D[1] })


## Problem 15
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return coldict2mat( {x: A * mat2coldict(B)[x] for x in B.D[1]})
...
def dot_prod_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return Mat( (A.D[0], B.D[1]), { (x,y):mat2rowdict(A)[x] * mat2coldict(B)[y] for x in A.D[0] for y in B.D[1] })
