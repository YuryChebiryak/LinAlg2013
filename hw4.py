...
linear_dep_R_2 = [-2.82842712474619,-0.45015815807855303,-1]
...
indep_vec_3 = Vec({0,1,2}, {0:0, 1:0, 1:-1})
...
def superset_basis(S, L):
    ...
    res = S.copy()
    for i in range(len(L)):
        if is_independent(res.copy() + [L[i]]):
            res.append(L[i])
    return res
...
def exchange(S, A, z):
    ...
    for w in W:
        # B - w
        check = [ x for x in B if x != w ]
        m = coldict2mat(check)
        s = solve(m, z)
        residual = z - m * s
        #if residual * residual > 10e-14:
        #    return False
        if not is_superfluous(check + [z], len(check)):
            return w
    return False
    #pass
