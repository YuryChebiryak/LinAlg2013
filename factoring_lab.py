...
def find_a_and_b(v, roots, N):
    ...
    '''
    alist = [ roots[x] for x in range(len(roots)) if v[x] != 0 ]
    a = prod(alist)
    #a = reduce(operator.mul, alist, 1)
    #c = reduce(operator.mul, [x **2 - N for x in alist], 1)
    c = 1
    for x in alist:
        c = c * (x * x - N)
    b = intsqrt(c)
...
#        if (a != -1) and (gcd(a-b, N) != 1) and (gcd(a-b, N) != N):
#            divisors.append(gcd(a-b, N))
#            print("divisor=", gcd(a-b,N))
